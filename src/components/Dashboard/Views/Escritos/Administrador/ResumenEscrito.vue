  ...
  <div class="content">
    <div class="container-fluid">
      <div class="row">
        ...
        <div class="col-md-8">
          <h5 style="font-size: 18px" class="card-title">Vista del Escrito</h5>
          <p class="card-category">Esta ventana muestra el escrito ingresado por nuestra plataforma.</p><br>
        </div>
        <div class="col-md-4">
...
var base64ToString = process.env.VUE_ENV === 'server' ? function(base64) { return Buffer.from(base64, 'base64').toString() } : window.atob;
  ...
  mounted: function () {
     ...
     this.nomSoli = this.$store.state.nomSolicitud;
     this.nidSoli = this.$store.state.nidSolicitud;
     this.verPdf(); 
  },
  components: {
     LTable,
  ...
  methods: { 
    ...
    verPdf: function () {
      if(this.nomSoli == null){
        this.$router.push('/listEscritos')
      }else{
      swal({
      ...
      this.pdfSrc='';
      this.page='';
      this.nombrePdf= this.nomSoli;
      axios.defaults.headers.common['Authorization'] = this.$session.get('token')
      axios.post(this.$store.state.serverOfi + '/cuadernillo/getPdfOV',
        {
        'path': this.nomSoli,
